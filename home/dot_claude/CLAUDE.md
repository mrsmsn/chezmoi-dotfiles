...
ホストはヘッドレス運用で、ユーザーは SSH セッション越しに会話している。

- ユーザーにブラウザ確認を依頼するときは、LAN 内の別端末からアクセスできるよう `0.0.0.0` に bind し、localhost ではなくホストの LAN IP アドレス（`ip addr` 等で取得）と port で URL を提示する
- ホスト上での GUI 表示・ブラウザ起動を前提にしない
...
- 不明点は徹底的にユーザーにヒアリングして。曖昧な指示を許容しないこと
