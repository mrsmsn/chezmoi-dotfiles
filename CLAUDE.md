...
- NixOS はシステム全体を NixOS が持つので、GUI/デスクトップ環境含むシステムレベルの設定は `nix/nixos/*.nix` (`configuration.nix` / `desktop.nix` / `fonts.nix`) に宣言する。ユーザー単位の GUI 設定は `nix/home/nixos.nix` (例: chromium — NixOS の `programs.chromium` は policy のみで本体を入れないため home-manager 側で宣言する)。
