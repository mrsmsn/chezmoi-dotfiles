...
- **flake input の更新は必ず `just nix-update`** を使う。素の `nix flake update` は (1) `llm-agents/nixpkgs` が numtide CI のビルド rev からずれて cache miss する、(2) root `nixpkgs` が niri-flake の pin から先行して niri が cache miss + nixpkgs 破壊的変更で eval が落ちる (`--override-input` での再 pin 2 本を just が自動化している)。root nixpkgs の鮮度は niri-flake の lock 更新頻度に律速される設計。
- substituter は `nix/flake.nix` の `nixConfig`、`nix/nixos/configuration.nix` と `nix/darwin/configuration.nix` の `nix.settings`、activate スクリプトの `--option` (darwin/nixos 両分岐)、`.github/actions/setup-nix/action.yml` (CI)、`install.sh` の `--extra-conf` (初回 bootstrap) の **6 箇所に同じ集合を登録**する。1 つでも欠けると初回に cache miss する (各箇所の役割分担は activate スクリプトのコメント参照)。flake nixConfig は実行ユーザーの trusted-settings.json 受諾状態に依存し、`darwin-rebuild` は HOME=~root にリセットするため、darwin では `--option` と `nix.settings` だけが決定的に効く。
