...
- substituter は `nix/flake.nix` の `nixConfig`、`nix/nixos/configuration.nix` と `nix/darwin/configuration.nix` の `nix.settings`、activate スクリプトの `--option` (darwin/nixos 両分岐) の **4 箇所に同じ集合を登録**する。1 つでも欠けると初回に cache miss する (各箇所の役割分担は activate スクリプトのコメント参照)。flake nixConfig は実行ユーザーの trusted-settings.json 受諾状態に依存し、`darwin-rebuild` は HOME=~root にリセットするため、darwin では `--option` と `nix.settings` だけが決定的に効く。
